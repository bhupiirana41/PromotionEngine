...
public class CalculatePromotion {
	...
	public static final int PRICE_COMBO_SKU_C_D = 30;
	public int total =0;
	...
	private int calculateSkuIdCAndD(int quantC, int quantD, int total) {
		...
		if(quantC >=1 && quantD >=1 ) {
			...
			total = (quantC == quantD ) ? total + quantC*PRICE_COMBO_SKU_C_D : total;
			
			total = (quantC > quantD ) ?  total + (quantD*PRICE_COMBO_SKU_C_D  + (quantC - quantD )*PRICE_SKU_C) : total ;
			
			total = (quantD > quantC ) ?  total + (quantC*PRICE_COMBO_SKU_C_D  + (quantD - quantC )*PRICE_SKU_D) : total;
